...
function slide(no) {
    ...
    $('.page').eq(slideNo).css({ 'opacity':'1', 'width':'150px' });
}
...
$('.hamburger').click(function(){  //햄버거메뉴(펼침메뉴)를 클릭했을 때
    ...
    $('#menu,#blackBox').fadeToggle(300);
});          

// 메인메뉴에 닿으면, 뒤 배경과 서브메뉴를 펼쳐보이기
$('.main, #subBack').mouseenter(function(){
    if( $(window).width() > 1000)
        $('#subBack').stop().slideDown(300);
});

// 메인메뉴에 떠나면, 뒤 배경과 서브메뉴를 접어 숨기기
$('.main, #subBack').mouseleave(function(){
    $('#subBack, .sub').stop().slideUp(300);
});
...
// 화면 폭 900px 넘으면 메뉴, 배경박스, 검색박스 초기화 
$(window).resize(function(){
